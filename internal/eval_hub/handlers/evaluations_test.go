...
func (f *fakeStorage) UpdateEvaluationJob(_ string, _ *api.StatusEvent) (api.OverallState, error) {
	return "", nil
}
...
func (s *updateEvaluationStorage) UpdateEvaluationJob(_ string, status *api.StatusEvent) (api.OverallState, error) {
...
func TestHandleCreateEvaluationMarksFailedWhenRuntimeErrors(t *testing.T) {
	...
	h := handlers.New(storage, validate, runtime, nil, nil, nil, nil)
...
func TestHandleUpdateEvaluationRewritesSidecarURLsInMessages(t *testing.T) {
	...
	h := handlers.New(storage, validate, &fakeRuntime{}, nil, nil, cfg, nil)
